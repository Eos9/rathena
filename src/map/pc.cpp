...
enum e_additem_result pc_additem(map_session_data *sd,struct item *item,int amount,e_log_pick_type log_type) {
	...
	if (id->type == IT_CHARM) status_calc_pc(sd, SCO_NONE); //dh	ATHENA
	/* rental item check */
	if( item->expire_time ) {
...
char pc_delitem(map_session_data *sd,int n,int amount,int type, short reason, e_log_pick_type log_type)
{
	...
	if( sd->inventory.u.items_inventory[n].amount <= 0 ){
		...
		mem = sd->inventory_data[n]->type;	// ATHENA
	...
	if (mem == IT_CHARM) status_calc_pc(sd, SCO_NONE);	// ATHENA
...
int pc_dead(map_session_data *sd,struct block_list *src)
{
		...
		&& !mapdata->getMapFlag(MF_NOEXPPENALTY) && !mapdata_flag_gvg2(mapdata))
	{
		...
		if (sd->sc.getSCE(SC_APACHE_LIFEINSURANCE))
			base_penalty -= (uint32)(a_base * (sd->sc.getSCE(SC_APACHE_LIFEINSURANCE)->val1 / 100.));		
		...
		a_job = job_penalty;
		if (sd->sc.getSCE(SC_APACHE_LIFEINSURANCE))
			job_penalty -= (uint32)(a_job * (sd->sc.getSCE(SC_APACHE_LIFEINSURANCE)->val1 / 100.));
